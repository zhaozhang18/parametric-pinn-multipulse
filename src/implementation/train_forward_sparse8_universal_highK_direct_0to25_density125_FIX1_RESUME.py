...
# 0-25 km domain is 1.25x longer than the original 0-20 km domain.
# Keep approximately the same PDE collocation density per unit z:
# 40,000 -> 50,000 points. Preserve the original per-K allocation ratios.
...
# Fine-tuning stage: same original High-K emphasis, with every per-K
# PDE budget multiplied by 1.25 to preserve z-domain point density.
...
def parse_args() -> argparse.Namespace:
    ...
    # Direct full-domain baseline: 0-25 km = 0-5 L_D.
    p.add_argument("--z-max-ld", type=float, default=5.0)
    ...
    p.add_argument("--n-pde", type=int, default=50000)
    p.add_argument(
        "--ensure-all-seen-ic",
    ...
    p.add_argument("--n-boundary", type=int, default=1280)
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Load --init-checkpoint and skip Adam/High-K stages, then run only L-BFGS. "
            "Use together with --completed-adam-steps and --completed-finetune-steps "
            "to reconstruct the exact final High-K collocation set."
        ),
    )
    p.add_argument("--completed-adam-steps", type=int, default=0)
    p.add_argument("--completed-finetune-steps", type=int, default=0)
...
def main() -> None:
    ...
    if args.init_checkpoint:
        ...
        init_payload = torch.load(str(init_path), map_location=device)
        if not isinstance(init_payload, dict) or "model_state" not in init_payload:
            raise RuntimeError(f"invalid init checkpoint: {init_path}")
        source_cfg = dict(init_payload.get("model_config", {}))
        target_cfg = dict(model.config())
        structural_keys = (
            "n_pulses",
            "hidden",
            "layers",
        ...
        source_struct = {k: source_cfg.get(k) for k in structural_keys}
        target_struct = {k: target_cfg.get(k) for k in structural_keys}
        if source_struct != target_struct:
            raise RuntimeError(
                "checkpoint architecture mismatch:\n"
                f"source={source_struct}\n"
                f"target={target_struct}"
            )
    ...
    if args.resume_lbfgs_only:
        if not args.init_checkpoint:
            raise ValueError("--resume-lbfgs-only requires --init-checkpoint")
        completed_adam = int(args.completed_adam_steps)
        completed_ft = int(args.completed_finetune_steps)
        if completed_ft > 0:
            every = int(args.finetune_resample_every)
            if every > 0:
                last_multiple = (completed_ft // every) * every
            else:
                last_multiple = 0
            if last_multiple >= 2:
                resume_seed_offset = completed_adam + last_multiple
            else:
                resume_seed_offset = completed_adam + 1
        else:
            resume_seed_offset = completed_adam + 1
        tensors, u0, v0 = resample(resume_seed_offset, "finetune")
        print(
            "resume L-BFGS collocation -> "
            f"stage=finetune, seed_offset={resume_seed_offset}, "
            f"completed_adam={completed_adam}, completed_finetune={completed_ft}",
            flush=True,
        )
    else:
    ...
    write_json(
        out_dir / "run_summary.json",
        {
            ...
            "resume_lbfgs_only": bool(args.resume_lbfgs_only),
            "completed_adam_steps_before_resume": int(args.completed_adam_steps),
            "completed_finetune_steps_before_resume": int(args.completed_finetune_steps),
        },
    )
